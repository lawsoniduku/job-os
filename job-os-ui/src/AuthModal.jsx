      ...
      <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
        ...
        {mode === "profile" ? (
          <form onSubmit={handleProfileSave} className="space-y-3">
            <div>
              <label className="text-xs text-zinc-500 block mb-1">Where are you searching from?</label>
              <select value={country} onChange={e => setCountry(e.target.value)}
                className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-zinc-600">
                {COUNTRY_OPTIONS.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
              </select>
              <p className="text-[11px] text-zinc-600 mt-1">Searches will automatically be filtered to roles open to this location.</p>
            </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {mode === "signup" && (
            <div>
              ...
              <p className="text-[11px] text-zinc-600 mt-1">We'll automatically filter searches to roles open to you. You can change this later.</p>
            </div>
          )}
